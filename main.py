...
class TicTacToe:
    ...
    def draw_winner(self):
        if self.winner:
            pg.draw.line(self.game.screen, 'green', *self.winner_line, CELL_SIZE // 8)
    ...
    def print_caption(self):
        ...
        if self.winner:
            pg.display.set_caption(f'Player "{self.winner}" wins! Press R to Restart')
        elif self.game_steps == 9:
            pg.display.set_caption(f'Game Over! Press R to Restart')
